...
- **确定递推公式**
  ...
  - 第一种情况：$dp[i - 1]$，上 $i-1$ 层楼梯，有 $dp[i - 1]$ 种方法，那么再一步跳一个台阶。
  - 第二种情况：$dp[i - 2]$，上 $i-2$ 层楼梯，有 $dp[i - 2]$ 种方法，那么再一步跳两个台阶。
  - 所以：
...
- **dp数组如何初始化**
  ...
  题目描述中明确说明"你可以选择从下标为 0 或下标为 1 的台阶开始爬楼梯。"也就是说到达第 0 个台阶是不花费的，但从第 0 个台阶往上跳的话，需要花费 $cost[0]$。所以初始化：
...
- **确定遍历顺序**
  ...
  $dp[i]$ 由 $dp[i-1]$ 和 $dp[i-2]$ 推出，所以是从前到后遍历 $cost$ 数组即可
...
- **确定递推公式**
  ...
  - 第一种情况：$j \times (i - j)$ 直接相乘。单纯的把整数拆分为两个数相乘
  - 第二种情况：$j \times dp[i - j]$，相当于是拆分 $(i - j)$，拆分成两个以及两个以上的个数相乘

  递推公式：

  $$
  dp[i] = \max(\{dp[i], \ (i - j) \times j, \ dp[i - j] \times j\})
  $$
...
- **确定遍历顺序**
  ...
  - 确定遍历顺序，递归公式：$dp[i] = \max(dp[i], \max((i - j) \times j, dp[i - j] \times j))$
  - $i$ 从 3 开始遍历到 $n$
  - $j$ 从 1 开始遍历到 $i-1$
...
- **确定dp数组（dp table）以及下标的含义**
  ...
  $dp[i]$：$i$ 个不同元素节点组成的二叉搜索树的个数。

- **确定递推公式**

  $dp[i] += dp[\text{以j为头结点左子树节点数量}] \times dp[\text{以j为头结点右子树节点数量}]$

  $j$ 表示选取的根节点结点的元素，从 1 遍历到 $i$ 为止。
  - 左子树：$[1, j-1]$
  - 右子树：$[j+1, i]$
  ...
  其中 $j-1$ 为 $j$ 为头结点左子树节点数量，$i-j$ 为以 $j$ 为头结点右子树节点数量。
...
- **确定遍历顺序**
  ...
  - 首先遍历节点数，从递归公式 $dp[i] += dp[j - 1] \times dp[i - j]$ 可以看出，节点数为 $i$ 的状态是依靠 $i$ 之前节点数的状态。
...
- **dp数组的初始化**
  ...
  - 首先 $dp[i][0]$ 一定都是 1，因为从 $(0, 0)$ 的位置到 $(i, 0)$ 的路径只有一条（竖直向下走），那么 $dp[0][j]$ 也同理（水平向右走）。
...
- **确定dp数组以及下标的含义**
  ...
  $dp[i][j]$ 表示从下标为 $[0-i]$ 的物品里任意取，放进容量为 $j$ 的背包，价值总和最大。

- **确定递推公式**

  - 第一种情:：不放物体则 $dp[i][j] = dp[i-1][j]$
...
- **dp数组的初始化**
  ...
  - $i=0$ 时：
    - $j < weight[0]$ 的时候，$dp[0][j]$ 是 0，因为背包容量比编号 0 的物品重量还小。
    - 当 $j \geq weight[0]$ 时，$dp[0][j]$ 是 $value[0]$
  - $j=0$ 时：
    - 背包容量为 0，$dp[i][0] = 0$
...
其实可以发现如果把 $dp[i - 1]$ 那一层复制到 $dp[i]$ 上，表达式完全可以是：
...
- **遍历顺序**
  ...
  - 顺序遍历物品，同时逆序遍历容量
  - **倒序遍历是为了保证物品 $i$ 只被放入一次**，从后往前循环，每次取得状态不会和之前取得状态重合，这样每种物品就只取一次了
...
- **dp数组的初始化**
  ...
  - $i=0$ 时：
    - 当 $j < weight[0]$ 的时候，$dp[0][j]$ 是 0，因为背包容量比编号 0 的物品重量还小。
    - 当 $j \geq weight[0]$ 时，$dp[0][j]$ 可以一直装物品 0，$dp[0][j] = dp[0][j - weight[0]] + value[0]$
  - $j=0$ 时：
    - 背包容量为 0，$dp[i][0] = 0$
...
同理 0-1 背包问题的一维 DP 解法，递推公式可以转换为：
...
- **遍历顺序**
  ...
  顺序遍历物品，同时顺序遍历容量（与 0-1 背包的区别在于容量是顺序遍历）
