...
<div align="center">
  <img src="72ab76ba697e470b8ceb14d5fc5688d9.png" alt="虚拟内存的使用" width="82%">
</div>
...
### ⏰ 映射的时机
- 虚拟内存和物理内存之间的映射通常在进程创建、内存分配（如malloc或mmap）、页面错误（Page Fault）发生时建立
- 解除映射则发生在内存释放（如free或munmap）、进程终止、内存页被换出到磁盘或地址空间被回收时。
...
<div align="center">
  <img src="a9ed979e2ed8414f9828767592aadc21.png" alt="虚拟地址的结构" width="82%">
</div>
...
<div align="center">
  <img src="c5e2ab63e6ee4c8db575f3c7c9c85962.png" alt="地址映射过程" width="82%">
</div>
...
<div align="center">
  <img src="6142bc3c917e4a6298bdb62936e0d332.png" alt="外部碎片" width="82%">
</div>
...
<div align="center">
  <img src="7884f4d8db4949f7a5bb4bbd0f452609.png" alt="地址翻译过程" width="82%">
</div>
...
<div align="center">
  <img src="edce58534d9342ff89f5261b1929c754.png" alt="程序的局部性原理" width="82%">
</div>
...
<div align="center">
  <img src="a3cdf27646b24614a64cfc5d7ccffa35.png" alt="TLB在CPU中的位置" width="82%">
</div>
...
<div align="center">
  <img src="f19ebd6f70f84083b0d87cc5e9dea8e3.png" alt="段页式内存管理" width="82%">
</div>
...
<div align="center">
  <img src="8904fb89ae0c49c4b0f2f7b5a0a7b099.png" alt="段页式地址变换数据结构" width="82%">
</div>
...
<div align="center">
  <img src="bc0aaaf379fc4bc8882efd94b9052b64.png" alt="地址转换过程" width="82%">
</div>
...
<div align="center">
  <img src="3a6cb4e3f27241d3b09b4766bb0b1124-20230309234553726.png" alt="内核空间与用户空间分布" width="82%">
</div>
...
- **代码段（Text Segment）**:进程运行之前，程序代码被编译成二进制文件存放在磁盘中。进程运行时，这些二进制文件中的机器码需要被加载进内存中，用于存放这些机器码的虚拟内存空间叫做**代码段**。

- **数据段（Data Segment）**:在程序代码中定义的全局变量和静态变量，在程序编译之后也会存储在二进制文件中。程序运行之前，这些全局变量也需要被加载进内存中供程序访问。那些**被我们指定了初始值的全局变量和静态变量**在虚拟内存空间中的存储区域叫做**数据段**。

- **BSS 段（BSS Segment）**:那些**没有指定初始值的全局变量和静态变量**在虚拟内存空间中的存储区域叫做**BSS 段**。这些未初始化的全局变量被加载进内存之后会被初始化为 0 值。因为内核知道这些数据是没有初值的，所以在二进制文件中只会记录 BSS 段的大小，在加载进内存时会生成一段 0 填充的内存空间。

- **堆（Heap）**:程序在运行期间往往需要动态的申请内存，在虚拟内存空间中也需要一块区域来存放这些动态申请的内存，这块区域就叫做**堆**。注意这里的堆指的是**操作系统堆**，并不是 **JVM** 中的堆。

- **文件映射与匿名映射区（Memory Mapping Segment）**:进程运行时所依赖的**动态链接库**（如 C 程序中的 glibc）以 `.so` 文件的形式存放在磁盘中，这些动态链接库也有自己的代码段、数据段、BSS 段，也需要一起被加载进内存中。还有用于**内存文件映射**的系统调用 `mmap`，会将文件与内存进行映射，那么映射的这块内存（虚拟内存）也需要在虚拟地址空间中有一块区域存储。这些动态链接库中的代码段、数据段、BSS 段，以及通过 `mmap` 系统调用映射的共享内存区，在虚拟内存空间的存储区域叫做**文件映射与匿名映射区**。

- **栈（Stack）**:在程序运行的时候总该要调用各种函数，那么调用函数过程中使用的**局部变量**和**函数参数**也需要一块内存区域来保存。这一块区域在虚拟内存空间中叫做**栈**。
...
<div align="center">
  <img src="ffb6e1727e2289f142f6a2a6291cd68c.png" alt="内核空间与用户空间分布" width="82%">
</div>
...
<div align="center">
  <img src="4956918c43e186d49df7b9802f080de8.png" alt="内核空间与用户空间分布" width="82%">
</div>
...
在动态内存分配中，当进程请求分配内存时，操作系统需要从空闲内存中选择合适的块来分配。常见的空闲块分配算法有三种：**首次适应（First Fit）**、**最佳适应（Best Fit）** 和 **最差适应（Worst Fit）**。

### 🎯 首次适应算法（First Fit）

**算法思想：** 从空闲链表的起始位置开始遍历，找到第一个足够大的空闲块就分配给进程。

**工作流程：**
1. 从空闲链表的头部开始依次检查每个空闲块
2. 如果当前空闲块的大小 `≥` 请求的大小，则分配该块
3. 如果空闲块剩余空间足够大，则分割成两块：一块分配给进程，一块保留为空闲块
4. 如果第一个块不够，则继续检查下一个空闲块

**示例：**

假设空闲链表包含以下空闲块（按地址顺序）：`[30KB] → [20KB] → [50KB] → [10KB]`

进程 A 请求分配 `15KB`：
- 检查第一个空闲块 `30KB ≥ 15KB`，分配！
- 分配后剩余 `15KB`，可以分割或全部分配

进程 B 请求分配 `25KB`：
- 检查第一个块 `15KB < 25KB`，不够，检查下一个
- 检查第二个块 `20KB < 25KB`，不够，检查下一个
- 检查第三个块 `50KB ≥ 25KB`，分配！
...
**缺点：**
- 容易产生小碎片（外部碎片）
- 频繁分配后，空闲链表前部会被小额空闲块占据

---

### 🎲 最佳适应算法（Best Fit）

**算法思想：** 找到最小但刚好满足需求的空闲块，使剩余的空闲块尽可能大。

**工作流程：**
1. 遍历整个空闲链表，记录所有满足大小要求的空闲块
2. 从候选块中选择**最小**的那个进行分配
3. 如果有剩余空间，则分割成小块保留在链表中

**示例：**

假设空闲链表包含：`[30KB] → [20KB] → [50KB] → [10KB]`
...
进程 B 请求分配 `15KB`：
- 满足条件的空闲块：`[30KB]`、`[10KB] + [5KB]`（合并后为 `15KB`）
- 选择最小的：`[15KB]`（合并后的块）
...
- 需要遍历整个空闲链表，分配速度慢
- 分割后会产生很多小碎片，难以再次利用
- 空闲链表需要按大小排序，或每次遍历时排序

---

### 🎯 最差适应算法（Worst Fit）

**算法思想：** 总是选择最大的空闲块进行分配，期望分配后剩余的空闲块仍然足够大。
...
|------|---------|---------|---------|
| **分配速度** | 快 | 慢 | 慢 |
| **碎片情况** | 中等外部碎片 | 最小外部碎片 | 减少大碎片 |
| **内存利用率** | 较好 | 最好 | 一般 |
| **实现复杂度** | 简单 | 较复杂 | 较复杂 |

**实际应用：**

- **First Fit** 是最常用的算法，实现简单且速度快
- **Best Fit** 在内存紧张时能更好地利用空间，但分配速度慢
- **Worst Fit** 较少使用，主要用于减少大碎片的产生

---

### 🔧 内存分配的实现方式

在实际的内存分配器（如 `malloc`/`free`）中，通常采用以下数据结构来管理空闲内存：
...
**2. 边界标签（Boundary Tags）**
每个内存块的头部和尾部都记录块的大小和状态，方便快速合并相邻空闲块：
...
**3. 内存池（Memory Pool）**
预先申请一大块内存，按固定大小分成多个 chunk，减少系统调用次数。

---

### 💡 选择建议

- **小内存分配**：优先使用 **First Fit**，速度快
- **大内存分配**：可以考虑 **Best Fit**，减少碎片
- **通用场景**：**First Fit** 是最佳选择，兼顾速度和利用率
