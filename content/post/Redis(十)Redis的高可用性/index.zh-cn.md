...
<div align="center">
  <img src="26f88373d8454682b9e0c1d4fd1611b4.png" alt="哨兵监控主从节点示意图" width="85%">
</div>
...
<div align="center">
  <img src="微信图片_2026-05-09_164929_244.png" alt="Redis哨兵故障转移流程" width="85%">
</div>
...
<div align="center">
  <img src="微信图片_2026-05-09_165026_807.png" alt="Redis集群模式示意图" width="85%">
</div>
...
例如发生了网络分区，主节点与哨兵、从节点分区了。此时哨兵发现联系不上主节点，于是发起选举，选了新的主节点，此时 Redis 就出现了两个主节点。这会导致：

- 客户端仍然连接到旧主节点并进行写操作，但这些写操作不会被哨兵和从节点同步
- 新主节点开始接收新的写操作
- 当网络分区恢复后，旧主节点会被降级为从节点，其上的写操作会被丢弃，造成数据不一致

**解决方案**：

在 Redis 中可以通过设置 `min-slaves-to-write` 和 `min-slaves-max-lag` 参数来缓解脑裂问题：

- `min-slaves-to-write`：主节点只有在至少有 N 个从节点连接时，才能执行写操作
- `min-slaves-max-lag`：主节点只有在至少有 N 个从节点的延迟不超过 M 秒时，才能执行写操作

这样可以保证在脑裂发生时，如果主节点失去了足够多的从节点连接，它会拒绝写操作，从而保护数据的一致性。
...
1. **主从模式**：适合读多写少的场景，通过主从复制实现数据备份和读写分离，但不具备自动故障转移能力
2. **哨兵模式**：在主从基础上增加了自动故障转移和通知功能，适合中等规模的集群，一般配置 3-5 个哨兵
3. **Cluster模式**：适合大规模数据存储场景，通过哈希槽分片实现数据分布式存储，支持自动故障转移和动态扩容

选择哪种方案需要根据实际业务需求（数据量、QPS、可用性要求等）进行权衡。