...
`init()` 是 Go 里非常典型的基础面试题。它的关键点不在“会不会写”，而在于你是否清楚它的**执行顺序**：
...
- 一个包里可以有多个 **`init()`**
- **`init()`** 没有参数，也没有返回值
- **`init()`** 不能被显式调用，只能由运行时在包初始化阶段触发

### 🙈 空白标识符 `_` 的常见用法

空白标识符 **`_`** 的作用是“接收但忽略”。最常见的使用场景有两个：
...
匿名导入的典型用途是注册驱动、注册编解码器、注册诊断能力等，因为这些能力往往在包的 **`init()`** 中完成。

### 🖨️ `fmt` 常见输出占位符

调试结构体时，下面三个格式化占位符最常见：
