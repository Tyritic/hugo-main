...
- 主键索引查询
  - 执行器第一次查询，会调用 read_first_record 函数指针指向的函数，因为优化器选择的访问类型为 const，这个函数指针被指向为 InnoDB 引擎索引查询的接口，把条件 交给存储引擎，**让存储引擎定位符合条件的第一条记录**。
...
- 全表扫描
  ...
  - 执行器会判断读到的这条记录的是不是符合条件，如果不是则跳过；如果是则将记录发给客户的（Server 层每从存储引擎读到一条记录就会发送给客户端，）。
