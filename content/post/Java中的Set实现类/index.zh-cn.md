...
- **并集（Union）** 并集是将两个集合的所有元素合并在一起，去重后返回新集合
  - `public boolean addAll(Collection<? extends E> c)`
  - 将指定集合中的所有元素添加到当前集合中
- **交集（Intersection）** 交集是将两个集合中共同的元素取出来
  - `public boolean retainAll(Collection<?> c)`
  - 只保留当前集合和指定集合中的交集部分
- **差集（Difference）** 差集是从一个集合中去除另一个集合中的元素
  - `public boolean removeAll(Collection<?> c)`
  - 从当前集合中移除与指定集合相交的元素，返回的是当前集合和指定集合的差集
...
- 对于字符串类型采用字典序进行排列
- 自定义规则
  - 让元素实现`Comparable`接口，重
  - 创建集合时传递`Comparator`指定gui'z