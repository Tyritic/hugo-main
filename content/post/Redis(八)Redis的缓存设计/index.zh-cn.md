...
### 对性能的影响
- **工作线程阻塞** ：由于 Redis 单线程执行命令，操作大 Key 时耗时较长，从而导致 Redis 出现其它命令阻塞的问题。
- **AOF阻塞** ：当使用 Always 策略的时候：如果写入是一个大 Key，主线程在执行 fsync() 函数的时候，阻塞的时间会比较久，因为当写入的数据量很大的时候，数据同步到硬盘这个过程是很耗时的。
- **客户端超时** ：因为操作大 Key 时耗时较长，可能导致客户端等待超时。
...
- 大化小，即把大对象拆分成小对象，即将一个大 Key 拆分成若干个小 Key，降低单个 Key 的内存大小。比如一个几万元素的Hash拆成多个小Hash，保证每个key的数据量在合理范围内。

- 采用 Redis 集群方式进行 Redis 的部署，然后将大 Key 拆分散落到不同的服务器上面，加快响应速度

- 定期清理过期数据，避免在Hash这类结构中不断追加数据却不清理失效的数据，导致内存占用过大。

<div align="center">
  <img src="微信图片_2026-05-09_171649_240.png" alt="Redis大Key解决方案" width="82%">
</div>
...
### 解决方案
- 在集群中复制热key。把热key foo复制成foo2、foo3、foo4等内容完全一样的key，分散到不同的分片上，请求也随机分散到这些副本上，降低单分片压力。
- 使用读写分离架构。如果热key主要是读请求，可以加从节点分担读压力。但这种方案会增加架构复杂度，需要加Proxy做负载均衡，从节点增多后故障率也会上升，运维成本更高。
- 本地缓存+分布式缓存多级缓存架构，减少对Redis的直接访问：在本地或其他缓存系统中存储热key的数据副本，减轻Redis的压力
...
实现原理是使用Redis SET命令的NX参数。当有多个线程进入时，我们就利用该方法，第一个线程进入时，redis 没有这个key 了，返回了1，如果结果是1，则表示他抢到了锁，那么他去执行业务，然后再删除锁，退出锁逻辑，没有抢到锁的，等待一定时间后重试即可。

- 加锁: SET lock_key uniqueValue EX expire_time NX
  - expire_time：锁需要有过期机制。假设某个客户端加了锁之后宕机了，锁没有设置过期机制，会使得其他客户端都无法抢到锁
  - uniqueValue：唯一标识，防止被别的线程给释放了
- 解锁: 通过Lua脚本先判断lock_key的值是不是自己的unique_value，是的话才del删除，保证原子性。

<div align="center">
  <img src="微信图片_2026-05-09_170051_342.png" alt="分布式锁设计" width="82%">
</div>

### 误删除问题的解决
只用setnx实现分布式锁有以下问题
- 持有锁的线程执行业务逻辑的时间超过了锁的过期时间，导致他的锁自动释放，这时其他线程，线程2来尝试获得锁，就拿到了这把锁，然后线程2在持有锁执行过程中，线程1反应过来，继续执行，而线程1执行过程中，走到了删除锁逻辑，此时就会把本应该属于线程2的锁进行删除
  - 解决方法：在释放锁的时候检查锁的所有者再进行删除
- 线程1现在持有锁之后，在执行业务逻辑过程中，他正准备删除锁，而且准备执行检查锁的拥有者的逻辑中，但是此时他的锁到期了，那么此时线程2进来获取锁，但是线程1他会接着往后执行，当他卡顿结束后，他直接就会执行删除锁那行代码，相当于判断锁的拥有者并没有起到作用，这就是删锁时的原子性问题，之所以有这个问题，是因为线程1的检查锁的拥有人和删锁，实际上并不是原子性的
  - 解决方法：利用lua脚本实现检查锁和删除锁的原子性，确保在释放锁的时候检查锁的所有者再进行删除
- 在分布式锁实现时，Setnx并不一定是可靠的，主要是由于主从复制不一致的情况可能发生。当一个客户端在主节点上成功执行Setnx命令获取锁时，但还没来得及同步到从节点上，此时主节点发生故障，从节点会被晋升为主节点，而此时之前获取锁的客户端在新的主节点上并没有获取到锁，导致出现锁失效的情况。
  - 解决方案
    - 采用Redlock算法来保证分布式锁的可靠性。Redlock算法是由Redis的作者提出的一种分布式锁算法，通过在多个独立的Redis实例上获取锁，并且在释放锁时要求大部分实例都成功释放才算释放成功
    - 主从延迟释放：加锁时先在主节点执行，休眠100ms(确保从节点同步)后再返回成功;解锁时先删主节点再删从节点

### 过期时间问题的解决
Redis实现分布式锁加了过期时间，业务代码还没执行完，锁就过期了怎么办？
- 看门狗机制：看门狗（watchdog）主要用来避免 Redis 中的锁在超时后业务逻辑还未执行完毕，锁却被自动释放的情况。它通过判断锁是否持有锁并定期刷新锁的过期时间来实现自动续期。
- 唯一标识：每个锁设置一个唯一值（比如 UUID），释放锁或续期时验证身份
