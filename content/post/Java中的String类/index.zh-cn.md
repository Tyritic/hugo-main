...
使用构造函数创建`String`对象的时候，先在**堆区**中创建一个新的`String`对象（引用常量池中的字面量），同时在字符串常量池中保存其字面量（字符串对象的引用）。若常量池中存在相同的字面值则只会在堆区中创建新的`String`对象![内存示意图](LT51yuh0_image_mianshiya.png)
