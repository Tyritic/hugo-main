...
- **Session** 技术基于**Cookie** 技术，继承了Cookies的全部缺点
...
- 第一部分: **Header** (头)，记录令牌类型以及所使用签名算法等。
- 第二部分：**Payload**(有效载荷)，携带所传递的数据。
- 第三部分：**Signature** (签名)，防止Token被篡改、确保安全性。将 **Header** 以及 **Payload** 和指定秘钥，通过指定签名算法计算而来。
...
2. 此后每个请求都要携带 **JWT** 令牌，服务端每次处理请求前都要校验令牌，校验通过后再处理
