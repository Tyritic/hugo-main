...
<div align="center">
  <img src="image-13.png" alt="源文中的 GC 可达对象与垃圾对象示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-12.png" alt="手动释放内存时复杂对象引用关系示意图" width="60%">
</div>

<div align="center">
  <img src="image-10.png" alt="错误释放对象导致悬挂引用示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-14.png" alt="引用计数法对象引用计数示意图" width="60%">
</div>

真正麻烦的是循环引用。两个对象即使业务上已经不用了，只要它们还彼此引用，单看计数就依然不为零。

<div align="center">
  <img src="image-11.png" alt="引用计数法循环引用问题示意图" width="60%">
</div>
...
<div align="center">
  <img src="image.png" alt="标记清除算法示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-1.png" alt="复制算法初始状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-2.png" alt="复制算法对象迁移示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-3.png" alt="标记整理算法整理前示意图" width="60%">
</div>

<div align="center">
  <img src="image-4.png" alt="标记整理算法整理后示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-16.png" alt="三色标记对象关系图" width="60%">
</div>

开始时，所有对象都是白色：

<div align="center">
  <img src="image-17.png" alt="三色标记初始全白状态示意图" width="60%">
</div>

扫描根对象，把第一批可达对象染成灰色：

<div align="center">
  <img src="image-18.png" alt="三色标记根扫描后灰色集合示意图" width="60%">
</div>

继续处理灰色对象，把它们引用到的白色对象染灰，并把自己染黑：

<div align="center">
  <img src="image-19.png" alt="三色标记处理中间状态示意图" width="60%">
</div>

重复这个过程，直到灰色集合为空：

<div align="center">
  <img src="image-20.png" alt="三色标记完成前最终状态示意图" width="60%">
</div>

最后剩下的白色对象就是垃圾，统一回收：

<div align="center">
  <img src="image-21.png" alt="三色标记完成后回收白色对象示意图" width="60%">
</div>
...
如果从 `runtime/mgc.go` 的视角去看，一轮 **Go GC** 大致会经历四个阶段：
...
- **Sweep Termination**：收尾上一轮清扫，准备进入新一轮 GC
- **Mark**：打开写屏障，把根对象入队，并发标记可达对象
- **Mark Termination**：短暂 STW，关闭标记相关工作，清理局部缓存
- **Sweep**：恢复正常分配，并发清扫未标记对象，把可用内存重新交还给分配器

这套流程和前面讲的“开始前短暂 STW + 并发标记 + Remark + 并发清扫”本质上是同一件事，只是这里更贴近运行时内部命名。
...
<div align="center">
  <img src="image-22.png" alt="并发三色标记中的初始危险状态示意图" width="60%">
</div>

这时，已经扫描完的黑色对象 `E` 新增了一个指向 `H` 的引用：

<div align="center">
  <img src="image-23.png" alt="黑色对象新增对白色对象引用示意图" width="60%">
</div>

同时，灰色对象 `F` 又删掉了原来指向 `H` 的那条边：

<div align="center">
  <img src="image-24.png" alt="灰色对象删除对白色对象引用示意图" width="60%">
</div>

结果就是：`H` 明明还被 `E` 引用着，但由于它不再处在灰色对象的后续扫描路径里，最终可能会被错误清理掉。

<div align="center">
  <img src="image-25.png" alt="并发三色标记发生错误回收示意图" width="60%">
</div>
...
Go 运行时里还有一个和写屏障强相关的结构叫 **`wbBuf`**。可以把它理解成每个 **P** 私有的小缓冲队列，写屏障先把需要处理的旧指针、新指针记进去，攒到一定数量后再批量刷新到扫描队列里，从而降低每次写屏障的即时开销。
...
如果你更习惯看源文里的原始配图，也可以对照下面这两张：

<div align="center">
  <img src="image-26.png" alt="源文中的强三色不变性示意图" width="60%">
</div>

<div align="center">
  <img src="image-27.png" alt="源文中的弱三色不变性示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-28.png" alt="插入写屏障初始状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-29.png" alt="插入写屏障下根扫描后的状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-30.png" alt="插入写屏障下的并发标记状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-31.png" alt="插入写屏障触发前的对象关系示意图" width="60%">
</div>

<div align="center">
  <img src="image-32.png" alt="插入写屏障把新对象置灰示意图" width="60%">
</div>

<div align="center">
  <img src="image-33.png" alt="插入写屏障继续标记示意图" width="60%">
</div>

不过单独依赖插入写屏障也有问题：**Go 不会在栈上普遍开启写屏障**。如果栈上的黑色对象新增了对白色对象的引用，就可能漏掉这次变化，因此早期实现需要在标记后期重新扫描栈。

<div align="center">
  <img src="image-34.png" alt="插入写屏障需要重新扫描栈示意图" width="60%">
</div>

<div align="center">
  <img src="image-35.png" alt="插入写屏障重新扫描栈进行补标示意图" width="60%">
</div>

<div align="center">
  <img src="image-36.png" alt="插入写屏障下重新扫描栈后的 STW 状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-37.png" alt="插入写屏障完成回收示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-38.png" alt="删除写屏障初始状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-39.png" alt="删除写屏障标记中间态示意图" width="60%">
</div>

<div align="center">
  <img src="image-40.png" alt="删除写屏障在替换引用时保护旧对象示意图" width="60%">
</div>

<div align="center">
  <img src="image-41.png" alt="删除写屏障在删除引用时标灰旧对象示意图" width="60%">
</div>

<div align="center">
  <img src="image-42.png" alt="删除写屏障完成后对象状态示意图" width="60%">
</div>

它能避免错误回收，但代价是**回收精度更保守**。某些已经逻辑失效的对象，因为在删除引用时被重新染灰，可能会多活一轮 GC。

另外，删除写屏障也不是完全不需要 **STW**。它依赖起始快照，也就是一开始先把根集合和第一层堆对象放进灰色保护之下，这一步如果不暂停所有 goroutine，就无法保证快照一致性。

<div align="center">
  <img src="image-43.png" alt="删除写屏障依赖起始快照示意图" width="60%">
</div>

如果试图一个栈一个栈地暂停，而不是一次性暂停全部 goroutine，也仍然会漏标：

<div align="center">
  <img src="image-44.png" alt="逐个暂停 goroutine 栈获取快照的危险状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-45.png" alt="逐个暂停栈导致错误回收示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-46.png" alt="混合写屏障初始状态示意图" width="60%">
</div>

<div align="center">
  <img src="image-47.png" alt="混合写屏障下扫描栈对象示意图" width="60%">
</div>

<div align="center">
  <img src="image-48.png" alt="混合写屏障下新栈对象直接视为存活示意图" width="60%">
</div>

<div align="center">
  <img src="image-49.png" alt="混合写屏障并发标记中间态示意图" width="60%">
</div>

<div align="center">
  <img src="image-50.png" alt="混合写屏障同时标灰旧值和新值示意图" width="60%">
</div>

<div align="center">
  <img src="image-51.png" alt="混合写屏障完成后的最终状态示意图" width="60%">
</div>
...
<div align="center">
  <img src="image-5.png" alt="切片缩容前的底层数组示意图" width="60%">
</div>

当切片缩成 `3` 个元素以后，业务上你确实“看不到”后半部分了，但底层数组仍然还挂在切片头上，因此从 **GC** 视角看，它依旧是可达的。

<div align="center">
  <img src="image-6.png" alt="切片缩容后底层数组仍可达示意图" width="60%">
</div>
