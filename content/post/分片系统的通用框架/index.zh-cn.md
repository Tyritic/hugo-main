...
<div align="center">
<img src="28f209cc-6566-11f1-96b4-463ce7757bbe.png" alt="分片三要素" width="82%">
</div>
...
<div align="center">
<img src="532b1832-6566-11f1-9fdf-6e86b87206bc.png" alt="分片函数" width="82%">
</div>
...
<div align="center">
<img src="64e62b52-6566-11f1-8f47-d271581afbe2.png" alt="布隆过滤器分片" width="82%">
</div>
...
```pseudocode
base_hash = hash(element) // 只算一次基础哈希
shard_index = base_hash % total_shards // 确定这个元素去哪个分片

// 在同一个分片内，用 7 个哈希函数算出不同偏移
// hash_i 等价于 hash(element + seed_i)，每个 seed 不同
offset_i = hash_i(element) % bits_per_shard
```

改动很小：用同一个 base_hash 决定分片归属，7 个 bit 全部落在同一个 Redis string 里。
...
<div align="center">
<img src="77f19024-6566-11f1-b126-a27132688e95.png" alt="网络路径对比" width="82%">
</div>
...
分片是系统扩展的必经之路，但分片的好坏决定了扩展的效率。掌握分片三要素——分片键、分片函数、分片映射——就能分析任何分片系统的核心设计。数据库分片需要在均匀性和扩展性之间权衡，而布隆过滤器分片则需要在存储效率和访问性能之间找到平衡。真正的艺术在于，根据具体场景选择合适的策略。
