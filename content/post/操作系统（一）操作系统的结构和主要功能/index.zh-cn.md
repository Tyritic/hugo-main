...
**操作系统**是为了管理计算机硬件和软件资源，提供用户与计算机交互的接口，并为应用程序的运行提供一个高效、安全和稳定的环境。

### 📋 设计目的

- **资源管理**：操作系统负责管理和分配计算机的硬件资源，例如 **CPU** 时间片、内存空间、磁盘存储等。它确保多个程序可以同时运行而不会相互干扰。
- **抽象化**：操作系统将复杂的硬件细节隐藏起来，为用户提供了一个更高层次的抽象。例如，**文件系统**将磁盘上的物理块抽象成文件和目录，用户无需关心具体的磁盘读写过程。
- **并发控制**：现代操作系统允许多个程序或线程同时运行，这需要操作系统提供**进程调度**、同步和互斥机制以避免冲突。
- **安全性**：操作系统提供访问控制和权限管理，防止未经授权的用户或程序访问敏感数据或资源。
- **用户接口**：无论是命令行界面还是图形用户界面，操作系统都提供了用户与计算机交互的方式。
- **系统调用支持**：操作系统为应用程序提供了一组**系统调用**，使得应用程序可以通过标准接口访问底层硬件资源，而不必直接操作硬件。

### 📌 核心功能

- **负责创建和终止进程**：**进程**是正在运行的程序实例，每个进程都有自己的地址空间和资源。
...
<div align="center">
  <img src="Kernel_Layout.png" alt="内核布局图" width="82%">
</div>
...
---
## 操作系统的内存分区
操作系统把内存分为内核空间(Kernel Space)和用户空间(User Space)。
- **内核空间（Kernel Space）**
  - 作用：供操作系统内核使用，包含内核代码、设备驱动程序、中断处理程序等。
  - 权限：用户程序不能直接访问，属于受保护的区域。
- **用户空间（User Space）**
  - 作用：供应用程序使用，包含应用程序代码、数据段等。
  - 权限：用户程序可以访问，属于用户自己的区域。
  - 具体分区
    - **代码段（Text Segment）**：存放程序的机器指令（即可执行代码）。通常是只读的，并且可以被多个进程共享。
    - **数据段（Data Segment）**：存放程序中初始化的全局变量和静态变量。可读写，不同进程各自拥有独立的副本。
    - **BSS段（Block Started by Symbol）**：存放未初始化的全局变量和静态变量。启动时系统会将其初始化为0。
    - **堆区（Heap）**：用于动态内存分配。内存从低地址向高地址增长。
    - **栈区（Stack）**：用于函数调用时存放局部变量、返回地址、参数等。每个线程有独立的栈，栈从高地址向低地址增长。
...
- **内核态**：在内核态下，CPU 可以执行所有的指令和访问所有的硬件资源。这种模式下的操作具有更高的权限，主要用于操作系统内核的运行。
- **用户态**：在用户态下，CPU 只能执行部分指令集，无法直接访问硬件资源。这种模式下的操作权限较低。应用程序需要通过系统调用与内核进行通信，由内核代为执行敏感操作。即使程序出现问题也不会影响操作系统的稳定性，主要用于运行用户程序。
...
<div align="center">
  <img src="systemcall.png" alt="内核态和用户态的切换" width="82%">
</div>
...
用户态和内核态频繁切换会带来一定的性能损耗：

- **保存与恢复 CPU 上下文**：包括寄存器、程序计数器（PC）、栈指针等。
- **切换页表或地址空间映射**：用户态和内核态可能使用不同的内存映射。
- **缓存失效（Cache Pollution）**：用户态和内核态使用不同的数据和代码，会导致 **CPU** 缓存命中率下降。
- **执行系统调用处理逻辑本身也消耗时间**。
...
系统调用是用户程序请求内核服务的一种方式，是从用户态切换到内核态的受控入口。

- **用户态发起系统调用**：用户态的程序发起系统调用，因为系统调用中涉及一些特权指令（只能由操作系统内核态执行的指令），用户态程序权限不足，因此会中断执行，也就是 Trap（Trap 是一种中断）。
- **用户调用库函数**：实际上不是直接调用内核，而是调用了 **glibc** 提供的封装函数。
- **glibc 调用 syscall 汇编指令**：**glibc** 中的 `read()` 等函数最终会调用底层的 `syscall` 汇编指令。
- **内核查找系统调用表**：**Linux** 内核维护了一个**系统调用号（syscall number）** 与函数指针的映射表。
- **执行内核函数**：内核函数在内核态中运行，可以操作硬件、管理内存、调度进程等。
- **返回用户态**：内核函数执行完成后，主动触发 Trap，这样会再次发生中断，会返回用户态，继续执行用户程序。
