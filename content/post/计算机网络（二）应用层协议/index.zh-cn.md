...
DHCP 协议的基本工作流程如下
- DHCP Discover：客户端设备向网络广播一个 DHCP Discover 请求包，询问是否有 DHCP 服务器可用。
- DHCP Offer：DHCP 服务器收到请求后，会向客户端广播一个 DHCP Offer 包，其中包含服务器可提供的 IP 地址和其他网络配置信息。
- DHCP Request：客户端选择一个 DHCP Offer，并发送一个 DHCP Request 包，表示它希望使用该 IP 地址和配置。
- DHCP Acknowledgement：DHCP 服务器确认请求，发送 DHCP Acknowledgement 包，完成配置过程。
