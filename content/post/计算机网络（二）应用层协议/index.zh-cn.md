...
## 📖 DNS协议
DNS 协议（Domain Name System）是一种用于将域名（如 www.example.com）转换为 IP 地址（如 192.168.1.100）的协议。它使得用户可以通过域名而不是 IP 地址来访问网站，而无需记住复杂的 IP 地址。

### 📋 DNS查询过程
- 递归查询（客户端 → 本地DNS服务器）：客户端只发送一个查询请求，由本地 DNS 服务器负责“跑完全程”，返回最终 IP。
- 迭代查询（本地DNS服务器 → 根 / 顶级域 / 权威DNS）：本地 DNS 不知道 IP 时，会逐级问上级 DNS，直到拿到结果。

- 本地DNS服务器检查自己的缓存，如果有记录则直接返回；如果没有，则开始执行迭代查询。
- DNS服务器向根域名服务器请求，根域名服务器会返回负责该域名的顶级域名服务器的地址。
- 然后DNS服务器向顶级域名服务器请求，顶级域名服务器会返回负责该域名的具体权威域名服务器的地址。
- 最后DNS服务器向权威域名服务器请求，权威域名服务器会返回具体的IP地址。
