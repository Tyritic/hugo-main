...
## ⚙️ DHCP协议
DHCP 协议 用于自动为网络中的设备分配 IP 地址 及其他网络配置参数（如子网掩码、默认网关、DNS 服务器等）。它使得设备无需手动配置 IP 地址就能自动获取并配置网络参数，是现代网络中非常常见的协议。

DHCP 协议的基本工作流程如下
- DHCP Discover：客户端首先发起 **DHCP 发现报文（DHCP DISCOVER）** 的 IP 数据报，由于客户端没有 IP 地址，也不知道 DHCP 服务器的地址，所以使用的是 UDP **广播** 通信，其使用的广播目的地址是 255.255.255.255（端口 67） 并且使用 0.0.0.0（端口 68） 作为源 IP 地址。DHCP 客户端将该 IP 数据报传递给链路层，链路层然后将帧广播到所有的网络中设备。
- DHCP Offer：DHCP 服务器收到DHCP 发现报文后，会向客户端广播一个 DHCP Offer 包，该报文仍然使用 IP 广播地址 255.255.255.255，该报文信息携带服务器提供可租约的 IP 地址、子网掩码、默认网关、DNS 服务器以及 **IP 地址租用期** 。
- DHCP Request：客户端收到一个或多个服务器的 DHCP 提供报文后，从中选择一个服务器，并向选中的服务器发送 **DHCP 请求报文（DHCP REQUEST** 进行响应，回显配置的参数，表示它希望使用该 IP 地址和配置。
- DHCP Acknowledgement：DHCP 服务器确认请求，发送 DHCP Acknowledgement 包，应答所要求的参数，完成配置过程。
- **再次租用**:如果租约的 DHCP IP 地址快期后，客户端会向服务器发送 DHCP 请求报文：
...
<div align="center">
  <img src="450px-HTTP-Header.png" alt="HTTP 基本工作模型" width="82%">
</div>
...
- 重定向机制的区别
  - **301 Moved Permanently** ：用于永久重定向，通常会更新客户端的书签。  • 告诉客户端和搜索引擎：资源 永久转移到新 URL。客户端以后请求会自动使用新 URL
  - **302 Found** ：用于临时重定向，不会更新书签。告诉客户端：资源临时在新 URL。客户端以后仍然可能使用旧 URL
...
<div align="center">
  <img src="HTTP请求协议.png" alt="HTTP 请求报文格式" width="85%">
</div>
...
<div align="center">
  <img src="image-20250209191221600.png" alt="HTTP 响应报文格式" width="85%">
</div>
...
<div align="center">
  <img src="16-短连接与长连接.png" alt="短连接与长连接" width="82%">
</div>
...
<div align="center">
  <img src="17-管道网络传输.png" alt="管道网络传输" width="82%">
</div>
...
- HTTP层请求队头阻塞：服务器是按请求的顺序响应的，如果服务器响应慢，会招致客户端一直请求不到数据
- 没有请求优先级控制
...
<div align="center">
  <img src="image-20240105143224839.png" alt="HTTP/2 底层实现" width="82%">
</div>
...
<div align="center">
  <img src="image-20240105143421889.png" alt="HTTP/2 Stream 示例" width="82%">
</div>
...
- TCP层队头阻塞：HTTP/2 多个请求是跑在一个 TCP 连接中的，那么当 TCP 丢包时，整个 TCP 都要等待重传，那么就会阻塞该 TCP 连接中的所有请求。
...
### HTTPS的解决思路
- 数据加密:HTTPS 使用 SSL/TLS 协议对数据进行加密 ：相较于HTTP协议明文传输，在数据传输过程中，数据会被加密成一串乱码，只有合法的接收方才能使用相应的密钥将其解密还原为原始数据。这就防止了数据在传输过程中被窃取或篡改，保护了用户的隐私和数据安全。
- 数字证书:HTTPS 要求服务器拥有由权威证书颁发机构颁发的数字证书 ：客户端在与服务器建立连接时，会验证服务器的证书，以确保连接的服务器是真实可靠的，而不是假冒的。这可以有效防止用户访问到钓鱼网站，避免用户的账号、密码等信息被窃取。
- 数据完整性:HTTPS 通过数字签名等技术来确保数据的完整性 ：在数据传输过程中，一旦数据被篡改，接收方就可以通过验证数字签名发现数据的变化，从而拒绝接受被篡改的数据，保证数据的准确性和完整性。
...
<div align="center">
  <img src="摘要算法.png" alt="摘要算法示意" width="75%">
</div>
...
<div align="center">
  <img src="数字签名.png" alt="数字签名过程" width="75%">
</div>
...
<div align="center">
  <img src="数字证书工作流程.png" alt="数字证书工作流程" width="82%">
</div>
...
<div align="center">
  <img src="640.png" alt="TLS 四次握手过程" width="85%">
</div>
...
<div align="center">
  <img src="5.jpg" alt="DNS 树状结构" width="85%">
</div>

### 🔄 域名解析过程
- 递归查询（客户端 → 本地DNS服务器）：客户端只发送一个查询请求，由本地 DNS 服务器负责“跑完全程”，返回最终 IP。
- 迭代查询（本地DNS服务器 → 根 / 顶级域 / 权威DNS）：本地 DNS 不知道 IP 时，会逐级问上级 DNS，直到拿到结果。

<div align="center">
  <img src="DNS-process.png" alt="迭代式域名解析过程" width="85%">
</div>
...
- 新电脑通常通过DHCP协议从网络中的DHCP服务器自动获取DNS服务器地址。如果未使用DHCP，则需要手动配置DNS服务器地址。
- 当电脑接入网络后，首先会广播一个DHCP请求，寻找网络中的DHCP服务器。
- DHCP服务器收到请求后，会分配一个IP地址，并提供其他网络配置信息，如子网掩码、默认网关和DNS服务器地址。
- 如果没有DHCP服务器，用户需要手动输入这些参数，包括DNS服务器的IP地址。

---

## 📡 RPC（**远程过程调用**）

**RPC**（**R**emote **P**rocedure **C**all），又叫做**远程过程调用**。它本身并不是一个具体的协议，而是一种**调用方式**。RPC 可以基于 HTTP 或 TCP 实现。相比 HTTP，RPC 通常采用二进制序列化，性能更高，适合微服务内部通信；而 HTTP REST 接口更通用、易调试，适合对外提供服务。
...
整个过程由代理机制、序列化、网络通信、服务发现和协议解析等组件协同完成。

核心思想是**客户端存根（Stub）**和**服务端骨架（Skeleton）**机制：
- 客户端调用一个本地的代理函数（Stub），该函数负责将参数序列化并发送到服务端。
- 服务端接收到请求后，由骨架（Skeleton）反序列化参数，调用实际的服务函数，执行完成后将结果序列化返回。
...
### ⚖️ WebSocket 和 SSE的区别
- 通信模型：WebSocket 是全双工、基于 TCP 的独立协议，支持客户端和服务端随时主动发送消息；而 SSE 是单向（服务端→客户端）、基于 HTTP 的轻量级事件流机制（text/event-stream），仅允许服务端推送数据，客户端无法通过同一连接发送消息。
- 底层协议
  - WebSocket 使用独立协议，需通过 HTTP Upgrade 请求完成握手。客户端发起 GET 请求，携带 Sec-WebSocket-Key 等头字段；服务端响应 101 Switching Protocols，并返回 Sec-WebSocket-Accept（由客户端 key 经固定算法 SHA-1 + base64 计算得出），之后连接脱离 HTTP，进入二进制/文本帧交换阶段。整个过程是协议升级（Protocol Upgrade），后续通信不再受 HTTP 请求-响应模型约束。
  - SSE 基于标准 HTTP：客户端使用 EventSource API 发起一个普通的 GET 请求，服务端响应状态码 200，并设置 Content-Type: text/event-stream，同时保持连接长期打开（Connection: keep-alive，且不发送 EOF）。服务端以特定文本格式（event:、data:、id:、retry:）分块写入响应体，浏览器自动解析并触发 message / error / event 类型事件。它本质上仍是 HTTP 流式响应，完全兼容 HTTP 缓存、代理、CDN（只要代理支持流式传输且不缓冲 chunked response）。
---
...
<div align="center">
  <img src="Tomcat目录结构.png" alt="Tomcat 目录结构" width="82%">
</div>