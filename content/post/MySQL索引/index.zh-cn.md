...
聚集索引就是索引结构和数据一起存放的索引，InnoDB 中的主键索引就属于聚簇索引。

在聚集索引中，它的叶子节点中包含整条记录，可以直接访问完整数据。在整个数据库有且只有一个



#### 非聚集索引

非聚集索引就是索引结构和数据分开存放的索引。MySQL 的 MyISAM 引擎，不管主键还是非主键，使用的都是非聚簇索引。

对于非聚集索引，它的叶子节点只包含主键值和索引值。
...
最左前缀匹配原则指的是在使用 **联合索引** 时，查询条件必须从索引的最左侧开始匹配。MySQL 会根据索引中的字段顺序，从左到右依次匹配查询条件中的字段。如果查询条件与索引中的最左侧字段相匹配，那么 MySQL 就会使用索引来过滤数据。最左匹配原则会一直向右匹配，直到遇到范围查询 **（如 >、<）** 就会停止匹配。**但是在范围查询字段的后面的字段无法用到联合索引。注意，对于 >=、<=、BETWEEN、like 前缀匹配的范围查询，并不会停止匹配**
