...
categories : ["Redis"]
tags : []
description : "深入解析 Redis List 数据类型，从底层实现原理（ziplist、quicklist）到常见命令，全面理解列表的应用场景。"
math : true
---
...
<div align="center">
  <img src="微信图片_2026-05-09_161328_134.png" alt="压缩列表示意图" width="82%">
</div>

- 优点：简单、紧凑、连续存储，适用于小数据量场景
- 缺点：压缩列表新增某个元素或修改某个元素时，如果空间不够，压缩列表占用的内存空间就需要重新分配。而当新插入的元素较大时，可能会导致后续元素的占用空间都发生变化，从而引起**连锁更新**问题，导致每个元素的空间都要重新分配，造成访问压缩列表性能的下降
...
<div align="center">
  <img src="list消息队列.png" alt="消息队列示意图" width="82%">
</div>
...
使用 LPUSH 作为队列生产者，使用 BRPOP 作为队列消费者，实现任务队列的异步处理。

### ⏱️ 排行榜实现

虽然 List 保有序特性，但不如 Sorted Set 灵活，通常用于记录最近的消息或操作日志。

---

## 📌 总结

List 是 Redis 中强大的数据结构，通过底层的 quicklist 实现既保证了操作性能又节省了内存。合理使用 List 可以实现消息队列、任务队列等场景，是构建高性能应用的重要工具。
